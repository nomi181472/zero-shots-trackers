...
Pipeline: GroundingDINO detects the prompt's classes -> the tracker (RAG by
default, ByteTrack/SORT/IoU available) associates them across frames.
...
def build_cli() -> argparse.ArgumentParser:
    ...
    # tracking
    p.add_argument("--tracker", default=None, choices=AVAILABLE_TRACKERS,
                   help="tracker to use (default: %(default)s)")
    ...
    # RAG retriever tuning
    p.add_argument("--w-appearance", type=float, default=None,
                   help="RAG: weight of appearance (CLIP) similarity")
    p.add_argument("--w-position", type=float, default=None,
                   help="RAG: weight of positional (IoU) similarity")
    p.add_argument("--memory-slots", type=int, default=None,
                   help="RAG: appearance embeddings kept per track memory")
    p.add_argument("--top-k", type=int, default=None,
                   help="RAG: retrieval candidates per detection")
...
def main() -> None:
    ...
    cfg = PipelineConfig(
        ...
        tracker_name=args.tracker or track_cfg.get("name", "rag"),
        ...
        w_appearance=args.w_appearance if args.w_appearance is not None
        else rag_cfg.get("w_appearance", 0.65),
        w_position=args.w_position if args.w_position is not None
        else rag_cfg.get("w_position", 0.35),
        memory_slots=args.memory_slots if args.memory_slots is not None
        else rag_cfg.get("memory_slots", 16),
        top_k=args.top_k if args.top_k is not None
        else rag_cfg.get("top_k", 8),
