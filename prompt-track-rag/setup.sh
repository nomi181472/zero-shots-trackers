...
# ---------------------------------------------------------------------------
# 6. Optional: pre-download the CLIP retrieval encoder for the RAG tracker
# ---------------------------------------------------------------------------
if [[ "${SKIP_CLIP:-false}" != "true" ]]; then
    echo "==> Pre-downloading CLIP encoder for the RAG tracker"
    python - <<'PY'
import os, sys
os.environ.setdefault("ZST_EMBEDDER", "clip")
try:
    from zero_shot_tracking.embedder import create_embedder
    emb = create_embedder("clip")
    print(f"CLIP embedder ready ({type(emb).__name__}, dim={emb.embedding_dim})")
except Exception as exc:  # no network / offline -> the app falls back anyway
    print(f"note: CLIP pre-download skipped ({exc}) — the app will fall back to "
          "the histogram embedder or fetch weights on first use")
PY
fi
