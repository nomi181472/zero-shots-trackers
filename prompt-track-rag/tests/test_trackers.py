...
os.environ.setdefault("ZST_EMBEDDER", "histogram")  # torch-free retrieval
...
@pytest.mark.parametrize("name", ["rag", "bytetrack", "sort", "iou"])
def test_registry_creates_all_trackers(name):
    trk = create_tracker(name)
    assert callable(trk.update)


def test_rag_tracker_needs_frame_flag():
    trk = create_tracker("rag")
    assert getattr(trk, "needs_frame", False) is True
...
def test_registry_maps_match_thresh_semantics():
    ...
    assert AVAILABLE_TRACKERS == ("rag", "bytetrack", "sort", "iou")