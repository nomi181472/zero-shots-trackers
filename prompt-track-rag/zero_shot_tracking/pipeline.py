...
Tracking defaults to the RAG tracker (:mod:`.rag`) but honors the runtime
tracker registry, exactly like the streaming backend.
"""
...
class ZeroShotTrackerPipeline:
    def __init__(self, cfg: PipelineConfig) -> None:
        ...
        self.tracker = create_tracker(
            ...
            w_appearance=cfg.w_appearance,
            w_position=cfg.w_position,
    ...
    def track_video(
        ...
        try:
            while True:
                ...
                # ---- track ----------------------------------------------------
                if getattr(self.tracker, "needs_frame", False):
                    tracks = self.tracker.update(dets, frame, text_prompt)
                else:
